...
sleep 2s
docker run --network pdf-net --rm --name=puppeteer -v="$PWD:/app" buildkite/puppeteer /bin/bash -c "cd app && wget http://$serverIp:4001/resume.html && npm install && npm start -- --url=http://$serverIp:4001/resume.html"
